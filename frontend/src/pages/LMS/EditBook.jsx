...
import { updateBook, getBookById } from "../../redux/book/bookSlice";
...
const EditBook = () => {
  ...
  useEffect(() => {
    ...
    // Prefill form fields when book data is available
    useEffect(() => {
        if (book) {
  ...
  const onSubmit = async (data) => {
    console.log('Book ID on submit:', id); 
    try {
        await dispatch(updateBook({ bookId: id, bookData: data })).unwrap();
